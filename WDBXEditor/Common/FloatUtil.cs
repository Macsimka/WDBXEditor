...
namespace WDBXEditor.Common
{
    ...
    {
        private static readonly int canonicalFloatNaN = FloatToRawIntBits(float.NaN);
        private static readonly int maxFloat = FloatToRawIntBits(float.MaxValue);
        private static readonly int minFloat = FloatToRawIntBits(float.MinValue);
        private static readonly int piFloat = FloatToRawIntBits((float)Math.PI);
        private static readonly int eFloat = FloatToRawIntBits((float)Math.E);
        private static readonly int piNegFloat = FloatToRawIntBits(-(float)Math.PI);
        private static readonly int eNegFloat = FloatToRawIntBits(-(float)Math.E);
        private static readonly int pInfinity = FloatToRawIntBits(float.PositiveInfinity);
        private static readonly int nInfinity = FloatToRawIntBits(float.NegativeInfinity);
